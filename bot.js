...
async function getUserInfo(accessToken, proxy) {
    try {
        ...
        // Ambil respons dalam bentuk JSON
        const data = await response.json();

        // Cek apakah objek data dan user ada
        if (data && data.user) {
            const userName = data.user.current_channel ? data.user.current_channel.title : 'Nama tidak tersedia';
            const walletAddress = data.user.ton_wallet_address || 'Tidak ada alamat wallet';
            
            // Periksa apakah channels ada dan apakah array ini memiliki setidaknya satu elemen
            if (Array.isArray(data.user.channels) && data.user.channels.length > 0) {
                const channelTitle = data.user.channels[0].title || 'Channel tidak tersedia';
                console.log(`Nama: ${userName}, Channel: ${channelTitle}, Alamat Wallet: ${walletAddress}`);
            } else {
                console.log(`Nama: ${userName}, Tidak ada channel yang tersedia, Alamat Wallet: ${walletAddress}`);
            }

            return data;
        } else {
            console.log("Error: Tidak ada data pengguna yang valid.");
            return null;
        }
    } catch (error) {
